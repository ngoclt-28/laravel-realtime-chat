...
class ConversationController extends \BaseController {
	...
	public function store() {
		...
		$conversation->users ()->attach ( array (
				Auth::user ()->id 
		) );
		...
		foreach ( Input::get ( 'users' ) as $user_id ) {
			array_push ( $messages_notifications, new MessageNotification ( array (
					'user_id' => $user_id,
					'read' => false,
					'conversation_id' => $conversation->id 
			) ) );
			...
			Event::fire ( ChatConversationsEventHandler::EVENT, array (
					json_encode ( $data ) 
			) );
		}
		...
		return Redirect::route ( 'chat.index', array (
				'conversation',
				$conversation->name 
		) );
	}
